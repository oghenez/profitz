...
namespace Profit
{
    public partial class SupplierOutstandingInvoiceForm : KryptonForm, IChildForm
    {
        SupplierOutStandingInvoice m_prn = new SupplierOutStandingInvoice(); 
        ...
        SupplierOutStandingInvoiceRepository r_soinv = (SupplierOutStandingInvoiceRepository)RepositoryFactory.GetInstance().GetJournalRepository(RepositoryFactory.SUPPLIER_OUTSTANDING_INVOICE_REPOSITORY);
        ...
        public SupplierOutstandingInvoiceForm(IMainForm mainForm, string formName)
        {
            InitializeComponent();
        ...
        void dataItemskryptonDataGridView_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            //    SupplierOutStandingInvoiceItem sample = new SupplierOutStandingInvoiceItem();
            //    sample.PART = p;
            //    sample.UNIT = u;
        ...
        private IList getItems()
        {
            ...
            for (int i = 0; i < itemsDataGrid.Rows.Count; i++)
            {
                if (itemsDataGrid[invoiceNoColumn.Index, i].Value == null) continue;
                if (itemsDataGrid[invoiceNoColumn.Index, i].Value.ToString() == "") continue;
                SupplierOutStandingInvoiceItem st = (SupplierOutStandingInvoiceItem)itemsDataGrid.Rows[i].Tag;
                if (st == null) st = new SupplierOutStandingInvoiceItem();
        ...
        private void loadData()
        {
            ...
            itemsDataGrid.Rows.Clear();
            foreach (SupplierOutStandingInvoiceItem item in m_prn.EVENT_JOURNAL_ITEMS)
            {
