...
namespace Profit
{
    public partial class ARCreditNoteForm : KryptonForm, IChildForm
    {
        ARCreditNote m_prn = new ARCreditNote(); 
        ...
        public ARCreditNoteForm(IMainForm mainForm, string formName)
        {
            InitializeComponent();
        ...
        void dataItemskryptonDataGridView_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            //        addedPI.Add(pi.ID);
            //    }
            //    IList res = r_grn.FindPObyPartAndGRNNo(e.FormattedValue.ToString(), addedPI, ((Customer)supplierkryptonComboBox.SelectedItem).ID, dateKryptonDateTimePicker.Value);
            //    if (res.Count == 0)
            //    {
            //        using (SearchGRNForPRForm fr = new SearchGRNForPRForm(e.FormattedValue.ToString(), ((Customer)supplierkryptonComboBox.SelectedItem).ID, addedPI, m_mainForm.CurrentUser, dateKryptonDateTimePicker.Value))
            //        {
            //            fr.ShowDialog();
            ...
            //    ARCreditNoteItem sample = new ARCreditNoteItem();
            //    sample.PART = p;
            //    sample.UNIT = u;
        ...
        private void InitializeDataSource()
        {
            ...
            //m_tops = r_top.GetAll();
            //m_employee = r_employee.GetAllSalesr();
        ...
        private IList getItems()
        {
            ...
            for (int i = 0; i < itemsDataGrid.Rows.Count; i++)
            {
                if (itemsDataGrid[invoiceNoColumn.Index, i].Value == null) continue;
                if (itemsDataGrid[invoiceNoColumn.Index, i].Value.ToString() == "") continue;
                ARCreditNoteItem st = (ARCreditNoteItem)itemsDataGrid.Rows[i].Tag;
                if (st == null) st = new ARCreditNoteItem();
        ...
        private void loadData()
        {
            ...
            itemsDataGrid.Rows.Clear();
            foreach (ARCreditNoteItem item in m_prn.EVENT_JOURNAL_ITEMS)
            {
                int i = itemsDataGrid.Rows.Add();
                itemsDataGrid.Rows[i].Tag = item;
                itemsDataGrid[invoiceNoColumn.Index, i].Tag = item.SALES_RETURN;
