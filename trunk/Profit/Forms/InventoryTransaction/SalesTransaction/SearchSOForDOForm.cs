...
namespace Profit
{
    public partial class SearchSOForDOForm : KryptonForm
    {
        ...
        public SearchSOForDOForm(string textfind, Customer supplier, IList added, User user, DateTime trDate)
        {
            InitializeComponent();
            m_user = user;
            m_trdate = trDate;
        ...
        {
            foreach (SalesOrderItem d in records)
            {
                ...
                gridData[topColumn.Index, row].Value = ((SalesOrder)d.EVENT).TOP.CODE;
                gridData[dueDateColumn.Index, row].Value = ((SalesOrder)d.EVENT).DUE_DATE.ToString("dd-MM-yyyy");
                gridData[codeColumn.Index, row].Value = d.PART.CODE;
                gridData[nameColumn.Index, row].Value = d.PART.NAME;
                gridData[qtyColumn.Index, row].Value = d.OUTSTANDING_AMOUNT_TO_DO;
        ...
        private void buttonSpecAny1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                IList records = r_po.FindSObyPartAndSONo(searchText.Text.Trim(), m_listAdded, m_customerID.ID, m_trdate);
                loadResult(records);
