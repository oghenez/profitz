...
namespace Profit
{
    public partial class SearchCussInvJForReceiptForm : KryptonForm
    {
        IList m_listAdded = new ArrayList();
        CustomerInvoiceJournalRepository r_po = (CustomerInvoiceJournalRepository)RepositoryFactory.GetInstance().GetJournalRepository(RepositoryFactory.CUSTOMERINVOICE_JOURNAL_REPOSITORY);
        ...
        public SearchCussInvJForReceiptForm(Currency ccy, Customer supplier, IList added, User user, DateTime trDate)
        {
            InitializeComponent();
            m_user = user;
            m_trDate = trDate;
            ...
            m_listAdded = added;
            m_ccy = ccy;
            IList records = r_po.FindCIJournalItemlistForReceipt("", m_ccy.ID, supplier.ID, m_trDate, added);
            loadResult(records);
        ...
        {
            foreach (CustomerInvoiceJournalItem d in records)
            {
                d.VENDOR = m_cusID;
                d.CURRENCY = m_ccy;
        ...
        private void buttonSpecAny1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                gridData.Rows.Clear();
                IList records = r_po.FindCIJournalItemlistForReceipt(searchText.Text.Trim(), m_ccy.ID, m_cusID.ID, m_trDate, m_listAdded);
                loadResult(records);
        ...
        private void AddResult()
        {
            for (int i = 0; i < gridData.Rows.Count; i++)
            {
                ...
                if (!check) continue;
                CustomerInvoiceJournalItem pi = (CustomerInvoiceJournalItem)gridData.Rows[i].Tag;
                RESULT.Add(pi);
            }
        }
