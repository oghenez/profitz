...
namespace Profit
{
    public partial class PriceCategoryForm : KryptonForm, IChildForm
    {
        PriceCategory m_pricecat = new PriceCategory();
        IMainForm m_mainForm;

        public PriceCategoryForm(IMainForm mainForm, string formName)
        {
            InitializeComponent();
        ...
        private void loadRecords()
        {
            try
            {
                ...
                gridData.Rows.Clear();
                IList records = RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.PRICE_CATEGORY_REPOSITORY).GetAll();
                foreach (PriceCategory d in records)
                {
        ...
        public void Delete(object obj, EventArgs e)
        {
            try
            {
                if (m_pricecat.ID > 0)
                {
                    ...
                    RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.PRICE_CATEGORY_REPOSITORY).Delete(m_pricecat);
                    removeRecord(m_pricecat.ID);
                    ClearForm();
                    setEnableForm(true);
