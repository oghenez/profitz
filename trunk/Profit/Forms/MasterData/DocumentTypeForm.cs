...
namespace Profit
{
    public partial class DocumentTypeForm : KryptonForm, IChildForm
    {
        DocumentType m_docType = new DocumentType();
        IMainForm m_mainForm;

        public DocumentTypeForm(IMainForm mainForm, string formName)
        {
            InitializeComponent();
        ...
        private void loadRecords()
        {
            try
            {
                ...
                gridData.Rows.Clear();
                IList records = RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.DOC_TYPE_REPOSITORY).GetAll();
                foreach (DocumentType d in records)
                {
        ...
        public void Save(object sender, EventArgs e)
        {
            try
            {
                if (Valid())
                {
                    ...
                    if (m_docType.ID == 0)
                    {
                        RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.DOC_TYPE_REPOSITORY).Save(m_docType);
                        DocumentType bank = (DocumentType)RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.DOC_TYPE_REPOSITORY).GetByCode(m_docType);
                        int r = gridData.Rows.Add(bank.CODE, bank.NAME);
                        gridData.Rows[r].Tag = bank;
                    }
                    else
                    {
                        RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.DOC_TYPE_REPOSITORY).Update(m_docType);
                        updateRecord();
                    }
        ...
        private void updateRecord()
        {
            ...
            {
                DocumentType dep = (DocumentType)item.Tag;
                if (dep.ID == m_docType.ID)
                {
                    gridData[0, item.Index].Value = m_docType.CODE;
                    gridData[1, item.Index].Value = m_docType.NAME;
                    break;
                }
            }
        }
        ...
        {
            m_docType.CODE = textBoxCode.Text.Trim();
            m_docType.NAME = textBoxName.Text.Trim();
            m_docType.MODIFIED_BY = m_mainForm.CurrentUser.NAME;
            m_docType.MODIFIED_COMPUTER_NAME = Environment.MachineName;
        }
        ...
        public void Delete(object obj, EventArgs e)
        {
            try
            {
                if (m_docType.ID > 0)
                {
                    ...
                    RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.DOC_TYPE_REPOSITORY).Delete(m_docType);
                    removeRecord(m_docType.ID);
                    ClearForm();
                    setEnableForm(true);
        ...
        private void gridData_SelectionChanged(object sender, EventArgs e)
        {
            ...
            ClearForm();
            m_docType = (DocumentType)gridData.SelectedRows[0].Tag;
            if (m_docType == null) return;
            loadData();
            setEnableForm(false);
        ...
        {
            textBoxCode.Text = m_docType.CODE;
            textBoxName.Text = m_docType.NAME;
        }
