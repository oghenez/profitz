            ...
            try
            {
                if (Valid())
                {
                    ...
                    if (m_bank.ID == 0)
                    {
                        ...
                        m_bank = (Bank)RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.BANK_REPOSITORY).GetByCode(m_bank);
                        int r = gridData.Rows.Add(m_bank.CODE, m_bank.NAME);
                        gridData.Rows[r].Tag = m_bank;
                    }
                    else
                    {
