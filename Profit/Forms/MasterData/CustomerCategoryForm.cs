...
namespace Profit
{
    public partial class CustomerCategoryForm : KryptonForm, IChildForm
    {
        CustomerCategory m_cuscat = new CustomerCategory();
        IMainForm m_mainForm;

        public CustomerCategoryForm(IMainForm mainForm, string formName)
        {
            InitializeComponent();
        ...
        public void Save(object sender, EventArgs e)
        {
            try
            {
                if (Valid())
                {
                    ...
                    if (m_cuscat.ID == 0)
                    {
                        RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.CUSTOMER_CATEGORY_REPOSITORY).Save(m_cuscat);
                        CustomerCategory bank = (CustomerCategory)RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.CUSTOMER_CATEGORY_REPOSITORY).GetByCode(m_cuscat);
                        int r = gridData.Rows.Add(bank.CODE, bank.NAME);
                        gridData.Rows[r].Tag = bank;
                    }
                    else
                    {
                        RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.CUSTOMER_CATEGORY_REPOSITORY).Update(m_cuscat);
                        updateRecord();
                    }
        ...
        {
            m_cuscat.CODE = textBoxCode.Text.Trim();
            m_cuscat.NAME = textBoxName.Text.Trim();
            m_cuscat.MODIFIED_BY = m_mainForm.CurrentUser.NAME;
            m_cuscat.MODIFIED_COMPUTER_NAME = Environment.MachineName;
        }
        ...
        public void Delete(object obj, EventArgs e)
        {
            try
            {
                if (m_cuscat.ID > 0)
                {
                    ...
                    RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.CUSTOMER_CATEGORY_REPOSITORY).Delete(m_cuscat);
                    removeRecord(m_cuscat.ID);
                    ClearForm();
                    setEnableForm(true);
        ...
        {
            textBoxCode.Text = m_cuscat.CODE;
            textBoxName.Text = m_cuscat.NAME;
        }
