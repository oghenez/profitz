...
namespace Profit
{
    public partial class SearchOstCussInvForReceiptForm : KryptonForm
    {
        IList m_listAdded = new ArrayList();
        CustomerOutStandingInvoiceRepository r_po = (CustomerOutStandingInvoiceRepository)RepositoryFactory.GetInstance().GetJournalRepository(RepositoryFactory.CUSTOMER_OUTSTANDING_INVOICE_REPOSITORY);
        public IList RESULT = new ArrayList();
        User m_user;
        Customer m_customerID;
        DateTime m_trDate;
        Currency m_ccy;

        public SearchOstCussInvForReceiptForm(Currency ccy, Customer supplier, IList added, User user, DateTime trDate)
        {
            InitializeComponent();
            m_user = user;
            m_trDate = trDate;
            m_customerID = supplier;
            m_listAdded = added;
            m_ccy = ccy;
            IList records = r_po.FindCOIJournalItemlistForReceipt("", ccy.ID, supplier.ID,m_trDate, added );
            loadResult(records);
        ...
        {
            foreach (CustomerOutStandingInvoiceItem d in records)
            {
                d.VENDOR = m_customerID;
                d.CURRENCY = m_ccy;
        ...
        private void buttonSpecAny1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                gridData.Rows.Clear();
                IList records = r_po.FindCOIJournalItemlistForReceipt(searchText.Text.Trim(), m_ccy.ID, m_customerID.ID, m_trDate, m_listAdded);
                loadResult(records);
