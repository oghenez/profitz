...
namespace Profit
{
    public partial class SearchSalesOrderForm : KryptonForm
    {
        ...
        Repository r_customer = RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.CUSTOMER_REPOSITORY);

        public SalesOrder SALES_ORDER = null;
        IList m_listLastresult = new ArrayList();

        public SearchSalesOrderForm(string textfind, IList result)//Point p)
        {
            InitializeComponent();
        ...
        private void gridData_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                SALES_ORDER = (SalesOrder)gridData.SelectedRows[0].Tag;
                this.Close();
            }
        }
