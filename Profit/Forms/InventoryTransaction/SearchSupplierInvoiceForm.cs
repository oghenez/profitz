...
namespace Profit
{
    public partial class SearchSupplierInvoiceForm : KryptonForm
    {
        SupplierInvoiceRepository r_po = (SupplierInvoiceRepository)RepositoryFactory.GetInstance().GetTransactionRepository(RepositoryFactory.SUPPLIERINVOICE_REPOSITORY);
        ...
        Repository r_supp = RepositoryFactory.GetInstance().GetRepository(RepositoryFactory.SUPPLIER_REPOSITORY);
        public SupplierInvoice SUPPLIER_INVOICE = null;
        IList m_listLastresult = new ArrayList();

        public SearchSupplierInvoiceForm(string textfind, IList result)//Point p)
        {
            InitializeComponent();
