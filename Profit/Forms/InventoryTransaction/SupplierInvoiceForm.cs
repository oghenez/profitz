...
namespace Profit
{
    public partial class SupplierInvoiceForm : KryptonForm, IChildForm
    {
        SupplierInvoice m_si = new SupplierInvoice(); 
        ...
        SupplierInvoiceRepository r_si = (SupplierInvoiceRepository)RepositoryFactory.GetInstance().GetTransactionRepository(RepositoryFactory.SUPPLIERINVOICE_REPOSITORY);
        ...
        public SupplierInvoiceForm(IMainForm mainForm, string formName)
        {
            InitializeComponent();
        ...
        void dataItemskryptonDataGridView_CellValidated(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == unitColumn.Index)
            {
                ...
                itemsDataGrid[priceColumn.Index, e.RowIndex].Value = r_si.GetTheLatestSIPrice(((Supplier)supplierkryptonComboBox.SelectedItem).ID, p.ID, u.ID);
                updateSubtotal(e.RowIndex);
            }
        ...
        void Post(object sender, EventArgs e)
        {
            try
            {
                ...
                //m_po = (SupplierInvoice)r_si.Get(m_po.ID);
        ...
        private void UpdateEntity()
        {
            ...
            m_si.SUPPLIER = (Supplier)supplierkryptonComboBox.SelectedItem;
        ...
        private void loadData()
        {
            ...
            supplierkryptonComboBox.Text = m_si.SUPPLIER.ToString();
            ...
            itemsDataGrid.Rows.Clear();
            foreach (SupplierInvoiceItem item in m_si.EVENT_ITEMS)
            {
        ...
        private void searchToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            if (result.Count == 1)
            {
                m_si = (SupplierInvoice)result[0];
                m_si = (SupplierInvoice)r_si.Get(m_si.ID);
            ...
            {
                using (SearchSupplierInvoiceForm frm = new SearchSupplierInvoiceForm(searchToolStripTextBox.Text, result))
                {
                    frm.ShowDialog();
                    if (frm.SUPPLIER_INVOICE == null)
                    {
                        return;
                    }
                    else
                    {
                        m_si = frm.SUPPLIER_INVOICE;
                        m_si = (SupplierInvoice)r_si.Get(m_si.ID);
                        m_si.EMPLOYEE = (Employee)r_employee.GetById(m_si.EMPLOYEE);
                        m_si.CURRENCY = (Currency)r_ccy.GetById(m_si.CURRENCY);
                        m_si.DIVISION = (Division)r_division.GetById(m_si.DIVISION);
